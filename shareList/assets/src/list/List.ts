...
import { Node, Component, Enum, tween, _decorator, EventHandler, Tween, ScrollView, Prefab, Layout, Vec2, Size, NodePool, isValid, instantiate, Vec3, Widget, UITransform, CCFloat, CCBoolean, CCInteger, v2 } from 'cc';
...
export default class List extends Component {
    ...
    //顶部刷新
    @property({})
    private topRefresh: boolean = false;

    //是否自由滑动
    @property({})
    private freeScroll: boolean = false;

    //底部刷新
    @property({})
    private downRefresh: boolean = false;
    ...
    _onScrolling(ev = null) {
        ...
        if ( (this.downRefresh || this.topRefresh) && ev) {
            this.checkTopRefresh(ev.getScrollOffset());
        }
        // this.adJustScorllDir( ev );
    ...
    private _refreshState:boolean = false;
    private _cb:any;
    private _target:any;
    private _isLockDir:boolean = false;
    private _touchStartPos:Vec2 = null;//按下时偏移

    public setRefreshCB(cb,target)
    {
        this._cb = cb;
        this._target = target;
    }

    public getAllItems():Array<Node>
    {
        return this.content.children;
    }

    // 刷新相关
    private checkTopRefresh(pos:Vec2)
    {
        if (!this._refreshState && !this._scrollView.isAutoScrolling()) {
            if (this.topRefresh) {
                if (pos.y < -30) 
                {
            ...
            } else if ( this.downRefresh ) {
                if (pos.y > this._scrollView.getMaxScrollOffset().y + 30 ) {
                    this._refreshState = true;
                    this.showTipText();
                }
            }
    ...
    private showTipText()
    {
        this._cb && this._cb.call(this._target,true);
    }

    // 隐藏提示文本
    private hideTipText()
    {
        if (this.topRefresh || this.downRefresh)
        {
            if (this._refreshState) {
                this._refreshState = false;
                this._cb && this._cb.call(this._target,false);    
            }
        }
        if (this.freeScroll) 
        {
    ...
    public resetAndLock()
    {
        ...
        {
            this.scrollView.scrollToOffset( v2(this._touchStartPos.x,this._touchStartPos.y ),0 );
    ...
    //调整滑动方向
    private adJustScorllDir(ev:ScrollView)
    {
        if ( this.freeScroll && !this._isLockDir && ev &&!ev.isAutoScrolling()) 
        {
            const ov = ev.getScrollOffset();
            const ox =  Math.abs(ov.x - this._touchStartPos.x);
            const oy =  Math.abs(ov.y - this._touchStartPos.y);
            if (ox > oy) {
                //hor
                ev.horizontal = true;
                ev.vertical = false;
                ev.scrollToOffset( v2(ov.x,this._touchStartPos.y ),0 );
            } else {
                //ver
                ev.horizontal = false;
                ev.vertical = true;
                ev.scrollToOffset( v2(this._touchStartPos.x,ov.y ),0 );
            }
            this._isLockDir = true;
        }
        
    }

    private lockStartPos()
    {
        this._isLockDir = false;
        this._touchStartPos = this.scrollView.getScrollOffset();
    }
